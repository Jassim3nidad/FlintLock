...
6. **Background prewarming — the case a manual cold launch (step 5) doesn't cover, because it still involves a user tapping the app icon.** iOS can start this app in the background with zero user interaction at all (launch-time-optimization prewarming, silent push handling, background App Refresh), including while the device has never been unlocked since boot. `AppDelegate.swift`'s `startReactNativeWhenDataIsAvailable()` is meant to prevent this from ever reaching the risky MMKV `mmap()` call at all, by checking `UIApplication.isProtectedDataAvailable` and deferring `factory.startReactNative()` until `attemptStartReactNative()` succeeds — showing `showWaitingForUnlockScreen()`'s plain native "Unlock your device to continue" view in the meantime rather than a frozen blank window (there is deliberately no timeout that starts React Native anyway; see `startReactNativeWhenDataIsAvailable`'s own doc comment for why). This needs confirming on a real device specifically because prewarming can't be triggered on demand from the app itself — it's an OS scheduling decision. Approaches, roughly in order of reliability: (a) force-quit the app, lock the device, leave it locked for an extended period, then check Xcode's device console / crash logs for any launch of the app process that occurred without a corresponding user-facing UI appearance; (b) use Xcode's own prewarming-simulation tooling if available for the target iOS version; (c) at minimum, confirm via logging that `protectedDataDidBecomeAvailableNotification` (trigger 1 of `attemptStartReactNative`'s two, see its doc comment) is reachable and correctly resumes by manually simulating "locked at launch" — lock the device immediately after tapping the app icon, before the app finishes launching, and confirm the waiting view actually appears (not a blank window), then confirm the app opens correctly (to the unlock screen) once the device is unlocked, rather than having crashed or hung during the gap.
7. **The opposite failure — `isProtectedDataAvailable` reading `false` on a device that's already unlocked and stays unlocked, so the fix for step 6 gets the diagnosis backwards.** Distinct from step 6 even though the *symptom* is identical (the waiting-for-unlock view appears): here the device was never locked to begin with, so `protectedDataDidBecomeAvailableNotification` — which only fires on a locked→unlocked *transition* — never fires either, and without the second recovery trigger this would be a genuine unbounded hang on an already-usable phone, not a brief, correct wait. `attemptStartReactNative()`'s second trigger (`applicationDidBecomeActive`, which fires on every foreground transition regardless of lock state) exists specifically to recover from this, but whether `isProtectedDataAvailable` can actually read `false` this way at all — a stale read at the exact instant a freshly-launched process's `didFinishLaunchingWithOptions` runs, before the OS has associated it with an already-unlocked session — is unconfirmed; reasoned as plausible, not verified, absent a real device. **Steps:** perform a normal, ordinary foreground launch (tap the icon on an unlocked, in-use device) many times in a row, ideally across different device/iOS-version combinations and under some memory/CPU pressure (other apps open, low storage) to maximize the chance of catching a launch-time race if one exists, and confirm the waiting view is never shown for a real, already-unlocked launch. If it ever is, confirm `applicationDidBecomeActive` actually recovers within one interaction (the user backgrounding and refocusing the app, or simply the OS's own post-launch active transition, should be enough) rather than requiring a force-quit.

If step 3, 4, 6, or 7 surfaces a crash, a silently-dropped write, a hang, or file corruption, that's a real defect in the file-protection-class choice or the prewarm guard, not a false alarm — see `SECURITY.md`'s "iOS file protection" bullet for the reasoning this test is meant to confirm or refute.

### 4. Android: biometric re-enrollment correctly disabling unlock — and, just as important, an ordinary cancel *not* doing the same

`nativeBiometricKeySource.ts`'s `isLikelyInvalidation()` treats `error.code === 'E_UNKNOWN_ERROR'` as "the enrolled key was invalidated by re-enrollment," traced from `react-native-keychain`'s Kotlin source, never confirmed against a real device. That code is an elimination remainder — react-native-keychain's Android bridge wraps a *declined/failed prompt* into a different code (`E_CRYPTO_FAILED`) before this check ever runs, and `E_UNKNOWN_ERROR` is whatever's left over after that and a keystore-access failure are both ruled out — not a code dedicated to confirmed invalidation. This makes it a two-sided claim, and both sides need checking, not just the one that makes the fix look like it worked:

- **A real invalidation must land in this branch.** If it doesn't — if some Android version or OEM keystore variant routes a genuine `KeyPermanentlyInvalidatedException` through a different code than this session's source-reading predicted — `unlock()` falls back to its old, pre-fix behavior: returns `null`, the button stays, and the user is back to tapping something that can never work again, with no explanation. A **false negative**.
- **An ordinary prompt cancellation must NOT land in this branch.** If tapping "cancel" (or a misread fingerprint, or backing out of the system prompt) is ever routed through `E_UNKNOWN_ERROR` rather than `E_CRYPTO_FAILED` — on some device/OS combination this session's source-reading didn't anticipate — the app would wrongly conclude the key is dead, silently remove the working Keychain entry via `disable()`, and turn `biometricUnlockEnabled` off. **The user loses their biometric enrollment for tapping "cancel."** A **false positive**, and the more dangerous of the two directions to miss in a device pass, since it actively destroys working state rather than just failing to fix broken state.

**Steps:**
1. On a real Android device (not an emulator, if avoidable — Keystore/biometric behavior is exactly the kind of thing emulators are known to diverge on), enable biometric unlock for a vault (Settings → biometric toggle → confirm master password).
2. **Cancellation check first, since it's the direction that can destroy state.** Lock the vault, tap "Unlock with biometrics," and cancel the system prompt (back button, tap outside the prompt, or let it time out without presenting a finger) — repeat a few times, including at least one genuine wrong-finger read if the device allows deliberately triggering one. After each: confirm the biometric-unlock button is **still present**, the vault is **still enrolled** (`VaultStore.peekSettings().biometricUnlockEnabled` is still `true` — check via a debug build or logging, not just the UI), and a **subsequent successful** biometric unlock still works. If the button disappears or the setting flips off after a mere cancel, that's F3's false-positive case confirmed on hardware — a real regression from this fix, not a false alarm.
3. **Invalidation check.** Go to the device's biometric settings (Settings → Security → Fingerprint/Face) and add a new enrollment (a different finger, or re-enroll the same one, whatever the OS allows) — the actual OS-level action `ACCESS_CONTROL.BIOMETRY_CURRENT_SET` is documented to invalidate against.
4. Return to Flintlock, lock the vault, tap "Unlock with biometrics." **Expected:** the specific message ("Your biometric enrollment changed, so biometric unlock was turned off...") appears, not the generic "cancelled or failed" one; the button disappears; `VaultStore.peekSettings().biometricUnlockEnabled` reads `false` afterward. If instead the generic message shows and the button remains (inviting another doomed tap), that's F3's false-negative case — the `E_UNKNOWN_ERROR` assumption didn't hold on this device, and `isLikelyInvalidation()` needs a wider or different signal.
5. Confirm the master password still unlocks the vault normally in both outcomes — the one guarantee that must hold regardless of how steps 2 or 4 turn out.

If step 2 or step 4 doesn't produce the branch this checklist expects, log the actual caught `error.code`/`error.message` (temporarily, via a debug build) before changing `isLikelyInvalidation()` — the fix should be re-aimed at what the device actually reports, not guessed at a second time.

 ...
 * Backed by the platform CSPRNG via CryptoProvider (OpenSSL RAND_bytes on
 * native, crypto.getRandomValues on web) — never Math.random().
 */
export function randomBytes(length: number): Buffer {
  return getCryptoProvider().randomBytes(length);
}

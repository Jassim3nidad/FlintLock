...
export interface VaultSettings {
   ...
   * vault (see packages/core/src/biometric). Tracked here, rather than
   * inferred by probing the platform biometric store, so checking it
   * never itself triggers a biometric prompt. The master password
   * remains the only recovery path regardless of this setting.
   */

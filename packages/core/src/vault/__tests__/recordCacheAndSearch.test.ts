...
describe('RecordCache', () => {
  it('returns the same decrypted data on repeated gets', async () => {
    ...
    const created = await createCredential(session, { ...BASE_INPUT, title: 'A', username: 'a', password: 'x' });
    const cache = new RecordCache(session);

    expect(await cache.get(created.id)).toEqual(created);
    expect(await cache.get(created.id)).toEqual(created);
  ...
  it('evicts the least-recently-used entry once past the bound', async () => {
    const session = await newSession();
    const a = await createCredential(session, { ...BASE_INPUT, title: 'A', username: 'a', password: 'x' });
    const b = await createCredential(session, { ...BASE_INPUT, title: 'B', username: 'b', password: 'x' });
    const c = await createCredential(session, { ...BASE_INPUT, title: 'C', username: 'c', password: 'x' });
    const cache = new RecordCache(session, 2);

    await cache.get(a.id);
    await cache.get(b.id);
    await cache.get(c.id); // evicts a (least recently used)
    ...
    expect(await cache.get(a.id)).toEqual(a);
    expect(cache.size).toBe(2);
  });
  ...
  it('recency: accessing an entry protects it from eviction', async () => {
    ...
    await cache.get(a.id);
    await cache.get(b.id);
    await cache.get(a.id); // bump a back to most-recently-used; b is now LRU
    await cache.get(c.id); // should evict b, not a

    expect(await cache.get(a.id)).toEqual(a); // still present, no re-decrypt needed to matter for this assertion
    expect(cache.size).toBe(2);
  });
...
describe('searchCredentials', () => {
  it('empty query returns all non-trashed credentials', async () => {
    const session = await newSession();
    await createCredential(session, { ...BASE_INPUT, title: 'Bank', username: 'a', password: 'x' });
    await createCredential(session, { ...BASE_INPUT, title: 'Email', username: 'b', password: 'x' });
    const cache = new RecordCache(session);

    expect((await searchCredentials(session, cache, '')).map((c) => c.title).sort()).toEqual(['Bank', 'Email']);
  });

  it('matches by title, username, or url, case-insensitively', async () => {
    const session = await newSession();
    await createCredential(session, { ...BASE_INPUT, title: 'GitHub', username: 'octocat', password: 'x', urls: ['https://github.com'] });
    await createCredential(session, { ...BASE_INPUT, title: 'Bank', username: 'someone', password: 'x' });
    const cache = new RecordCache(session);

    expect((await searchCredentials(session, cache, 'github')).map((c) => c.title)).toEqual(['GitHub']);
    expect((await searchCredentials(session, cache, 'OCTOCAT')).map((c) => c.title)).toEqual(['GitHub']);
  });
  ...
  it('excludes hidden custom fields from the search haystack', async () => {
    ...
    expect(await searchCredentials(session, cache, 'unfindable-token')).toEqual([]);
  });
  ...
  it('matches visible (non-hidden) custom fields', async () => {
    ...
    expect(await searchCredentials(session, cache, 'findable-token')).toHaveLength(1);
  });
});

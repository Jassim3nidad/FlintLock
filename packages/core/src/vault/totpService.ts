...
export async function createTotpEntryManually(session: UnlockSession, input: ManualTotpInput): Promise<TotpEntry> {
  ...
  await session.vault.putRecord(entry);
  return entry;
}

/** otpauth:// URI path — from a QR scan or pasted text. */
export async function createTotpEntryFromOtpauthUri(session: UnlockSession, uri: string, credentialId: string | null): Promise<TotpEntry> {
...
export async function getTotpEntry(session: UnlockSession, id: string): Promise<TotpEntry | undefined> {
  const record = await session.vault.getRecord(id);
  return record?.recordType === 'totp' ? record : undefined;
}

export async function listTotpEntries(session: UnlockSession, options: { includeTrashed?: boolean } = {}): Promise<TotpEntry[]> {
  const includeTrashed = options.includeTrashed ?? false;
  const index = await session.vault.listIndex();
  const entries = await Promise.all(
    index.filter((entry) => entry.recordType === 'totp').map((entry) => getTotpEntry(session, entry.id))
  );
  return entries.filter((e): e is TotpEntry => e !== undefined && (includeTrashed || e.deletedAt === null));
}

export async function getTotpEntriesForCredential(session: UnlockSession, credentialId: string): Promise<TotpEntry[]> {
  return (await listTotpEntries(session)).filter((e) => e.credentialId === credentialId);
}
 ...
 */
export async function getCurrentCode(session: UnlockSession, entry: TotpEntry, unixTimeSeconds?: number): Promise<CurrentCode> {
  if (entry.mode === 'totp') {
  ...
  const code = await generateHotp(base32Decode(entry.secret), entry.counter, {
    algorithm: entry.algorithm,
    digits: entry.digits,
  });
  await session.vault.putRecord({ ...entry, counter: entry.counter + 1, updatedAt: Date.now() });
...
export async function updateTotpEntry(
  session: UnlockSession,
  id: string,
  changes: Partial<Pick<TotpEntry, 'issuer' | 'account' | 'credentialId'>>
): Promise<TotpEntry> {
  const existing = await getTotpEntry(session, id);
...
export async function softDeleteTotpEntry(session: UnlockSession, id: string): Promise<void> {
  const existing = await getTotpEntry(session, id);
  if (!existing) throw new Error(`No TOTP entry with id ${id}`);
  await session.vault.putRecord({ ...existing, deletedAt: Date.now(), updatedAt: Date.now() });
}

export async function hardDeleteTotpEntry(session: UnlockSession, id: string): Promise<void> {

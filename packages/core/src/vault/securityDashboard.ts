 ...
 * generator's own entropy figure (packages/core/src/generator), which is
 * *exact* because it controls the random process end to end. This
 * function is guessing at the strength of a string someone else chose —
 * it has no way to know if "Tr0ub4dor&3"-style substitutions came from a
 * dictionary word, whether it's a keyboard walk, or whether it's reused
 * from a famous breach. A real strength meter (zxcvbn-style, pattern- and
 * dictionary-aware) is out of scope here; this is deliberately a coarse,
 * explainable floor, not a claim of precision.
 */
 ...
 */
export async function analyzeSecurity(session: UnlockSession, unixTimeMs: number = Date.now()): Promise<SecurityDashboardReport> {
  const credentials = await listCredentials(session);
  const credentialIdsWithTwoFactor = new Set(
    (await listTotpEntries(session)).filter((entry) => entry.credentialId !== null).map((entry) => entry.credentialId)
  );

...
import { generateSalt } from '../crypto/envelope';
import { deriveKek } from '../crypto/kdf';
import { getCryptoProvider } from '../platform';
import { GCM_AUTH_TAG_LENGTH_BYTES, GCM_IV_LENGTH_BYTES, KdfParams } from '../crypto/types';
...
export async function encodeFlbx(masterPassword: Buffer, payloadJson: string, kdfParams: KdfParams): Promise<Buffer> {
  ...
  let envelope;
  try {
    envelope = await aesGcmEncrypt(Buffer.from(payloadJson, 'utf8'), key, flbxAad());
  } finally {
    provider.disposeKey(key);
  }
 ...
 * (from packages/core/src/crypto) for anything the GCM auth tag catches
 * — wrong password, a tampered ciphertext, or a ciphertext truncated
 * past the structural minimum. Either way: never returns partial data —
 * the caller only ever gets a value from this function after every check
 * has fully passed.
 */
export async function decodeFlbx(masterPassword: Buffer, file: Buffer): Promise<string> {
  ...
  if (!Buffer.from(file.subarray(0, FLBX_MAGIC.length)).equals(FLBX_MAGIC)) {
  ...
  try {
    header = JSON.parse(Buffer.from(file.subarray(headerStart, headerEnd)).toString('utf8')) as FlbxHeader;
  } catch {
  ...
  const iv = Buffer.from(file.subarray(ivStart, tagStart));
  const authTag = Buffer.from(file.subarray(tagStart, ciphertextStart));
  const ciphertext = Buffer.from(file.subarray(ciphertextStart));

  const provider = getCryptoProvider();
  const salt = Buffer.from(header.salt, 'base64');
  const key = await deriveKek(masterPassword, header.kdf, salt);
  try {
    const plaintext = await aesGcmDecrypt({ iv, authTag, ciphertext }, key, flbxAad());

...
describe('exportCsv', () => {
  it('requires acknowledgeRisk: true', async () => {
    ...
    await expect(exportCsv(session, { acknowledgeRisk: false as unknown as true })).rejects.toThrow(/acknowledgeRisk/);
  });
  ...
  it('resolves tagIds to tag names', async () => {
    ...
    await createCredential(session, { ...BASE_INPUT, title: 'X', username: 'a', password: 'p', tagIds: [tag.id] });
  ...
  it('neutralizes formula-injection prefixes with a leading apostrophe', async () => {
    const session = await newSession();
    await createCredential(session, { ...BASE_INPUT, title: '=cmd|"/c calc"!A1', username: '+1', password: '@evil', notes: '-1' });

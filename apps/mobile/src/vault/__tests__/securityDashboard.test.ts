...
describe('analyzeSecurity — weak passwords', () => {
  it('flags a password below the entropy threshold and not one above it', async () => {
    ...
    const weak = createCredential(session, { ...BASE_INPUT, title: 'Weak', username: 'a', password: 'abc123' });
    const strong = createCredential(session, {
      ...BASE_INPUT,
      title: 'Strong',
      username: 'b',
    ...
    const report = analyzeSecurity(session);
...
describe('analyzeSecurity — reused passwords', () => {
  it('groups credentials sharing an identical password and ignores unique ones', async () => {
    const session = await newSession();
    const a = createCredential(session, { ...BASE_INPUT, title: 'A', username: 'a', password: 'shared-pw-123' });
    const b = createCredential(session, { ...BASE_INPUT, title: 'B', username: 'b', password: 'shared-pw-123' });
    createCredential(session, { ...BASE_INPUT, title: 'C', username: 'c', password: 'unique-pw-456' });
...
describe('analyzeSecurity — old passwords', () => {
  it('flags a password older than the retention threshold and not a recent one', async () => {
    const session = await newSession();
    const old = createCredential(session, { ...BASE_INPUT, title: 'Old', username: 'a', password: 'x' });
    const recent = createCredential(session, { ...BASE_INPUT, title: 'Recent', username: 'b', password: 'y' });
    ...
    session.vault.putRecord({
      ...(session.vault.getRecord(old.id) as Credential),
...
describe('analyzeSecurity — missing two-factor', () => {
  it('flags a credential with no attached TOTP entry and not one that has one', async () => {
    const session = await newSession();
    const withTotp = createCredential(session, { ...BASE_INPUT, title: 'HasTOTP', username: 'a', password: 'x' });
    const withoutTotp = createCredential(session, { ...BASE_INPUT, title: 'NoTOTP', username: 'b', password: 'y' });
...
describe('analyzeSecurity — trashed credentials are excluded entirely', () => {
  it('does not surface a soft-deleted credential in any finding category', async () => {
    const session = await newSession();
    const trashed = createCredential(session, { ...BASE_INPUT, title: 'Gone', username: 'a', password: 'abc123' });
    softDeleteCredential(session, trashed.id);

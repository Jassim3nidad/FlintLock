 ...
 */
export function exportKeePassXml(session: UnlockSession, options: KeePassXmlExportOptions): string {
  if (options.acknowledgeRisk !== true) {
    throw new Error('KeePass XML export produces a plaintext file — acknowledgeRisk must be true');
  }

  const tagsById = new Map(listTags(session).map((tag) => [tag.id, tag.name]));
  const entries = listCredentials(session).map((credential) => {
    ...
    return [
      '    <Entry>',
      fields,
      tagNames ? `      <Tags>${xmlEscape(tagNames)}</Tags>` : null,
      '    </Entry>',
    ]

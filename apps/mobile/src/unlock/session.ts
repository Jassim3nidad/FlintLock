 ...
 * to VaultStore.lock(), which wipes the DEK — this class never holds any
 * key material of its own, only a reference to the store.
 *
...
export class UnlockSession {
  ...
  /** For the biometric-unlock path — see src/biometric/biometricVault.ts. */
  unlockWithDek(dek: Buffer): void {
    this.store = VaultStore.openWithDek(dek);
    this.scheduleAutoLock();
  }

...
import { TOTP_VECTORS } from '../__fixtures__/totp.vectors';

describe('generateTotp — RFC 6238 Appendix B known-answer vectors', () => {
  for (const v of TOTP_VECTORS) {
    it(`t=${v.unixTimeSeconds} ${v.algorithm} -> ${v.expectedCode}`, () => {
      const secret = Buffer.from(v.secretHex, 'hex');
      const code = generateTotp(secret, { algorithm: v.algorithm, digits: 8, period: 30 }, v.unixTimeSeconds);
...
describe('generateTotp — default digits/period behave sanely', () => {
  it('produces a 6-digit code with default SHA1 params', () => {
    const secret = Buffer.from('12345678901234567890', 'utf8');
    const code = generateTotp(secret, { algorithm: 'SHA1', digits: 6, period: 30 }, 59);
    expect(code).toHaveLength(6);
  });

  it('the same second-window produces the same code, and adjacent windows differ', () => {
    const secret = Buffer.from('12345678901234567890', 'utf8');
    const params = { algorithm: 'SHA1' as const, digits: 6 as const, period: 30 };
    // Window 3 spans [90, 119]; window 4 starts at 120.
    const a = generateTotp(secret, params, 100);
    const b = generateTotp(secret, params, 119); // same 30s window as 100
    const c = generateTotp(secret, params, 120); // next window
    expect(a).toBe(b);
    expect(a).not.toBe(c);
  });
});

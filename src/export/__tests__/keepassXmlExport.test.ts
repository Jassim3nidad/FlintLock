...
describe('exportKeePassXml', () => {
  it('requires acknowledgeRisk: true', async () => {
    ...
    expect(() => exportKeePassXml(session, { acknowledgeRisk: false as unknown as true })).toThrow(/acknowledgeRisk/);
  });
  ...
  it('marks the Password field Protected', async () => {
    const session = await newSession();
    createCredential(session, { ...BASE_INPUT, title: 'X', username: 'a', password: 'hunter2' });
    const xml = exportKeePassXml(session, { acknowledgeRisk: true });
  ...
  it('includes tag names as a semicolon-joined Tags element', async () => {
    ...
    createCredential(session, { ...BASE_INPUT, title: 'X', username: 'a', password: 'p', tagIds: [t1.id, t2.id] });
  ...
  it('omits the Tags element entirely for a credential with no tags', async () => {
    const session = await newSession();
    createCredential(session, { ...BASE_INPUT, title: 'X', username: 'a', password: 'p' });
    const xml = exportKeePassXml(session, { acknowledgeRisk: true });

 ...
 * demand search approach proposed in src/storage/vaultStore.ts's header
 * comment. Backed by a Map: insertion order doubles as recency order,
 * since re-inserting a key on access moves it to the end.
 *
 ...
 * decrypted vault state can survive after the DEK is wiped. "Locking
 * wipes the DEK and all decrypted state from memory" (spec §5.5)
 * includes this cache, not just the DEK.
 */
export class RecordCache {
  ...
  get(id: string): VaultRecord | undefined {
    ...
    const record = this.session.vault.getRecord(id);

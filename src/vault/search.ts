 ...
 * src/storage/vaultStore.ts. `customFields` marked `hidden` are excluded
 * from the search haystack, per spec.
 */
export function searchCredentials(session: UnlockSession, cache: RecordCache, query: string): Credential[] {
  const q = query.trim().toLowerCase();
  const results: Credential[] = [];

  for (const entry of session.vault.listIndex()) {
    if (entry.recordType !== 'credential') continue;
    const record = cache.get(entry.id) as Credential | undefined;
